...
\end{trditev}
\begin{proof}
    Poglejmo si najprej poseben primer, ko je $m = 1$. Na eni strani (v množici $V_m$) imamo torej samo eno vozlišče, ki je stopnje $n$, na drugi strani pa $n$ vozlišč stopnje 1 (opazimo, da so vozlišča iz $V_n$ ekvivalentna v smislu, da ni važno, katero točno je pobarvano, saj imajo vsa isto stopnjo in soseda). Če črno pobarvamo vsa vozlišča v $V_n$ razen enega, bo eno izmed teh vozlišč poskrbelo za spremembo barve vozlišča stopnje $n$, to pa bo poskrbelo za spremembo barve edinega vozlišča stopnje 1, ki še ni črno. Velja torej $Z(K_{1,n}) \leq 1 + n - 2 = n-1$. Poskusimo najti manjšo množico ničelne prisile. Na začetku lahko pobarvamo $n-2$ vozlišča na dva načina: lahko pobarvamo vozlišče stopnje $n$ in $n-3$ vozlišč stopnje 1, lahko pa pobarvamo $n-2$ vozlišč stopnje 1. Obravnavajmo oba primera.
    \begin{enumerate}[a)]
