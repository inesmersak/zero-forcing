...
\label{subsec:pred-graf-rz}

Ker si bomo v delu pogledali tudi nekaj algoritmov, ki imajo za vhodni podatek graf, si na kratko oglejmo načine predstavitve grafov v računalnikih in zahtevnosti operacij pri različnih predstavitvah. 
...
V delu se bomo dotaknili tudi težavnosti odločitvenih problemov, zato si poglejmo nekaj osnovnih definicij računske zahtevnosti.
\begin{definicija}
    \begin{enumerate}
    ...
    \end{enumerate}
\end{definicija}
Povedano drugače, v razred $\P$ spadajo tisti problemi, za katere poznamo algoritem, ki najde rešitev v polinomskem času. Po drugi strani v $\NP$ spadajo tisti problemi, za katere poznamo algoritem, ki v polinomskem času preveri, ali je dana rešitev pravilna. 
...
\hl{TODO: ekivalentna definicija NP s certifikatom}
...
V prejšnjem podpoglavju smo si pogledali algoritem z eksponentno časovno zahtevnostjo za reševanje problema števila ničelne prisile za splošen graf. Ali lahko najdemo hitrejši, polinomski algoritem? V tem poglavju bomo dokazali, da je problem števila ničelne prisile $\NP$-težek; če bi torej obstajal polinomski algoritem za naš problem, bi sledilo $P = NP$.
...
Na odločitveni problem (označimo ga z \ZFOdl) ga prevedemo, če se namesto minimizacije velikosti množice ničelne prisile vprašamo: ali za dan graf $G$ in dano število $k \in \N$ obstaja množica ničelne prisile velikosti $k$? Če je ta odločitveni problem $\NP$-poln, potem je \textsf{ZF-Opt} problem $\NP$-težek -- če ne poznamo polinomskega algoritma, ali obstaja množica ničelne prisile neke velikosti, potem tudi ne poznamo polinomskega algoritma, ki nam bi vrnil najmanjšo. 

Dokazali bomo sledeč izrek:
\begin{izrek}
    Problem \ZFOdl\ je $\NP$-poln za ravninske grafe z utežmi. \hl{TODO}
\end{izrek}
Spomnimo se definicije $\NP$-polnosti~(\ref{}): veljati mora, da je odločitveni problem v $\NP$ in, da se da vsak problem iz $\NP$ v polinomskem času prevesti na tega. Da je \ZFOdl v $NP$, vemo

dokaz v NP s certifikatom, ki je polinomske dolžine in preverljiv v polinomskem času
