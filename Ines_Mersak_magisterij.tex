...
Nekaj o tem, da so vozlišča ponavadi tudi kar indeksi, zato se bolj kot ne vse dela z arrayi ali seznami.
...
\hl{TODO: Če imamo MNP in dodamo kakšno vozlišče, je to še vedno MNP. Potem se skliči na to pri eksponentnem algoritmu.}
\hl{TODO: V posebnem je tudi unija dveh MNP MNP. Presek ni, protiprimer cikel.}
...
Analizirajmo zahtevnost algoritma~\ref{alg:eksponentni-izracun-zf}. Za vsak element potenčne množice~$V$ pokličemo \textsc{preveriIzboljsan}, ki ima časovno zahtevnost $O(m+n)$. Vseh elementov je $2^n$, skupna časovna zahtevnost algoritma je torej $O(2^n(m+n))$. Prostorska zahtevnost algoritma pa je $O(m+n)$, saj je to prostorska zahtevnost funkcije  \textsc{preveriIzboljsan}, ki edina porabi nekaj dodatnega prostora (ki ga po izhodu iz funkcije tudi sprosti), preostanek algoritma pa porabi konstantno prostora.

Razmislimo, ali lahko algoritem~\ref{alg:eksponentni-izracun-zf} kako izboljšamo. Hitro opazimo, da nam pravzaprav ni treba pregledati čisto vseh podmnožic. (Komentar: ni čisto isto, a gremo od dol ali od gor, od gor lahko takoj breakamo, če kakšno najdemo. Also, lahko pa samo podmnožice neke veljavne MNP pregledujemo.)

Lahko izboljšamo z bisekcijo?
