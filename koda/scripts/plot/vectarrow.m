function vectarrow(p0,p1,color,style)
...
%   $Revision: 1.0
  ...
  elseif max(size(p0))==2
      if max(size(p1))==2
          ...
          x1 = p1(1);
          y1 = p1(2);
          plot([x0;x1],[y0;y1],'color',color,'linestyle',style);   % Draw a line between p0 and p1
          
          p = p1-p0;
          alpha = 0.10;  % Size of arrow head relative to the length of the vector
          beta = 0.10;  % Width of the base of the arrow head relative to the length
          ...
          plot(hu(:),hv(:),'color',color)  % Plot arrow head
          %grid on
          %xlabel('x')
          %ylabel('y')
          hold off
