function handles = plot_arrow( x1,y1,x2,y2,HeadSize,Scale,varargin )
hold on;
%
%DODATEK by KOSA
%ce je HeadSize==0 potem naredi autoscale, drugace je pa velikost glave puscice dolocen s to sprem.
%s Parametrom scale skaliras dolzino puscice 
...
if(HeadSize==0) HeadSize=line_length; end
head_length = min( HeadSize*alpha,max_length );
head_width  = min( HeadSize*beta,max_length );

% SKALIRANJE LINIJE
% ================================================\

x2=x1+line_length*cos(teta+pi/2)*Scale;
y2=y1+line_length*sin(teta+pi/2)*Scale;
